...
- [Jira Test Management migrator (JTMM)](#jira-test-management-migrator-jtmm)
  ...
  - [Product overview](#product-overview)
    - [Jira Server vs Jira Cloud](#jira-server-vs-jira-cloud)
    - [ADO Cloud vs ADO Server](#ado-cloud-vs-ado-server)
  - [Configuration file](#configuration-file)
    - [Example from Zephyr Squad](#example-from-zephyr-squad)
    - [Map-fields](#map-fields)
  - [User Config File](#user-config-file)
    - [user_config.txt example](#user_configtxt-example)
    - [Credentials values set in a .env file](#credentials-values-set-in-a-env-file)
  ...
  - [How to run JTMM](#how-to-run-jtmm)
    ...
    - [Logging](#logging)
    - [Running together with Jira to Azure DevOps Migrator PRO](#running-together-with-jira-to-azure-devops-migrator-pro)
  - [Individual documentation](#individual-documentation)
    - [QMetry](#qmetry)
    - [Xray](#xray)
    - [Zephyr Scale | Zephyr Squad](#zephyr-scale--zephyr-squad)
  - [Preserving Links to Jira Issues via Azure DevOps Work Items](#preserving-links-to-jira-issues-via-azure-devops-work-items)
    - [Configuring Jira Azure DevOps Migrator PRO](#configuring-jira-azure-devops-migrator-pro)
    - [Configuring Jira Test Management Migrator](#configuring-jira-test-management-migrator)
...
| `mapFields`          | Specify field to field or field value mapping in this block, mapFields is exaplained [here](#map-fields)                        |          | zephyr squad, xray, Testflo |
...
```json
{
    ...
    "mapFields": {
        "Test": [
            ...
            {
                "source": "Sprint",
                "target": "Custom.Sprint",
                "type": "arrayToTextField"
            },
            {
                "source": "custom field",
                "target": "Custom.CustomField",
                "type": "textFieldToTextField",
                "mapper":{
                    "value 1": "1",
                    "value 2": "2",
                    "value 3": "3"
...
### Map-fields
This config attricbute enables you to map Jira fields to specified ADO fields. You can either enter the Jira `customfield_id` or the name of the field.
...
```
This example maps the Jira field `custom field` to the ADO field `Custom.CustomField`. If the Jira value has the value `value 1`, it will map to the value `1`.

#### Allowed mapping types
| Type | Description |
|------|-------------|
| `rendered` | Map HTML/rendered fields. The ADO field must be of type `Multi line` |
| `arrayToTextField` | Converts array values to comma-separated text in ADO |
| `textFieldToTextField` | Maps normal string/int fields to ADO's text fields  |
| `mapper` | Maps the field's value to a target value |

#### Supported platforms for map fields
| Platforms | Supported |
|------|-------------|
| `xray-server` | ✅ |
| `xray-cloud` | ✅ |
...
| `ZEPHYR_ACCESS_KEY`     | Zephyr API Access Key (platform = `zephyr-squad`, `only cloud`)     |
