...
| --ado-wiki-id-override | \* | ADO Wiki ID (manual override). Setting this value will disable any lookups to the ADO Rest API. | --ado-wiki-id-override "b2cddbe-6999-4cd4-b3ed-6148e52af597" |
...
- During the **Confluence export** and **transformation** steps, the machine will need a stable connection and access to the **Confluence server** or **Confluence cloud**.
- During the **ADO import** (git push) step, the machine will need a stable connection and access to the **ADO Server** or **ADO Services**.

You do not need access to ADO during the export, and similarly you do not need access to Confluence during the import.

If you do not have a stable connection to Confluence during the export, the tool will not be able to run.
...
#### **Reason 1**: The file was not included in the HTML export
...
#### **Reason 2**: Notice about script file encoding
...
#### **Reason 3**: Invalid value of --confluence-space-name
...
#### **Reason 4**: Page is not migrated due to view+edit restrictions
...
#### **Reason 5**: The title in the .html file does not match the page title or the space name

A common symptom of this problem is that only some pages are migrated, and others not.

If you inspect the .html file of a page which is not correctly migrated, you will find the `<title>` block, for example:

```html
<!DOCTYPE html>
<html>
    <head>
        <title>Product Management Team : My Page</title>
```

The contents of this `<title>` block must exactly match the **Space name** and the **Page name** with a colon and spaces (" : ") in between:

- **Space name**: Product Management Team
- **Page name**: My Page

If the **Space name** or the **Page name** do not match, the tool may silently fail to export this page, or it may print an error message such as this one:

```
Could not find <title> block in Page C:\solidify\Source\2700214274.html. Perhaps this page was created with the legacy editor?
```

Here are some examples of `<title>` blocks which will cause the export to fail:

- `<title>Product Management Team: My Page</title>`
- `<title>Product Management Team - My Page</title>`
- `<title>Product Management Team: My Other Page</title>`
- `<title>My Other Space: My Page</title>`

#### **Reason 6**: **system longpaths** registry key is disabled on the system

This will cause the tool to fail when writing to filepaths longer than 250 characters. Please enable the registry key, reboot the system and try again. (<https://learn.microsoft.com/en-gb/windows/win32/fileio/maximum-file-path-limitation?tabs=registry#enable-long-paths-in-windows-10-version-1607-and-later>)

#### **Reason 7**: The turndown service ran out of memory

Check wether or not the `turndown.log` file ends with this line:

```
[I] turndown.js complete!
```

If the above line is not present in the `turndown.log` file, the turndown service might be exiting prematurely due to running out of RAM.

If this is the case, you should see an error message in the terminal like this one: `'JavaScript heap out of 'memory'`.

To rule out wether this is the issue or not, try and set the `--node-memory-size` parameter to a value higher than default (2048), like `--node-memory-size 4096`, or a value (in megabytes) which is larger than the size of the Source directory. If this results in more files being written, then you will need to try and run again and again while increasing the `--node-memory-size` until the turndown service terminates as expected.
...
#### **Other recommendations for troubleshooting SSL issues**

- Can you call the jira rest api manually from the same computer?
- VPN problems?
- You may need to add a trusted root certificate to the correct cert store
- Is the computer/network locked down?
