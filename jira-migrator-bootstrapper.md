...
| `--issue-replay-stop-at`       | During issue replay, how many issues should we maximum replay? Set this to a lower value if the program does not terminate within a reasonable time. Doing so might mean that not all available users and states are reported. Default = all issues in the project  | int            |
...
### My **users-generated.txt** file does not contain all of the users who have contributed to my project

The bootstrapper will only be able to find the users which the current user can read from the Rest API. If the permissions of the user running the bootstrapper are not sufficient to read the details of the user in the changelog from the Rest API, the tool may omit reporting some users. It might be worth trying elevated permissions (project admin) or asking someone else with higher permissions to create a token and try.

You can also try and change the parameter `--issue-replay-stop-at 1000` to a higher value or omit it altogether in order to ensure that the tool enumerates more data (Warning: This will cause longer run times). With this parameter set, the tool may omit older issues for the sake or terminating sooner.

In the worst case, you can ignore this problem and run JiraExport.exe with the (incomplete) users-generated.txt file that you get. JiraExport.exe will warn about all users that were found in any issue changelog but are not mapped, so you can manually map them accordingly.
